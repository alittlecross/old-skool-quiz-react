...
const Menu = ({ game, io }) => {
  ...
  for (const [key] of Object.entries(game.players)) {
    ...
    cells[0].push(<MenuTableCell key={key} {...props(0, key, '✓')} />)
    cells[1].push(<MenuTableCell key={key} {...props(1, key, '#')} merge={merge} />)
    cells[2].push(<MenuTableCell key={key} {...props(2, key, '✗')} />)
  }

  return (
    <div id='menu'>
      <span className='highlight-yellow' id='switch-host' onClick={e => handleClick(e, 0)}>Switch host...</span>
      &nbsp;
      <span className='highlight-blue' id='merge-players' onClick={e => handleClick(e, 1)}>Merge players...</span>
      &nbsp;
