...
const PastQuestions = ({ cookie, game, handleClick, io, updateCookie }) => {
  const arr = []
    ...
    arr.push(
      <div key={i}>
        <div className='question'>
          {i + 1}. {q.question} <span className='highlight-yellow'>{Array.isArray(q.answer) ? q.answer[q.correct] : q.answer}</span>
        </div>
